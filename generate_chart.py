...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .slider-wrap {{ display: flex; align-items: center; gap: 10px; margin-bottom: 16px; }}
  .slider-wrap input[type=range] {{ flex: 1; max-width: 420px; accent-color: #222; }}
...
<div class="slider-wrap">
  ...
  <input type="range" id="date-slider" min="0" max="100" value="0" step="1" oninput="onSlider(this.value)">
  <span class="slider-label" id="slider-date-label">Oct 22</span>
  <button onclick="resetSlider()" style="font-size:11px;padding:3px 8px;">Reset</button>
</div>
...
let sliderMin = 0;  // current x-axis start index

function formatSliderDate(idx) {{
  return allDates[idx] ? new Date(allDates[idx]).toLocaleDateString('en-US', {{ month: 'short', day: 'numeric' }}) : '';
}}

function onSlider(val) {{
  sliderMin = parseInt(val);
  document.getElementById('slider-date-label').textContent = formatSliderDate(sliderMin);
  if (chart) {{
    chart.options.scales.x.min = sliderMin;
    chart.update('none');
    // Update legend to reflect current endpoint values
...
function resetSlider() {{
  ...
  document.getElementById('slider-date-label').textContent = formatSliderDate(0);
  if (chart) {{
...
function buildChart(conf) {{
  ...
  // Keep slider in sync with date range
  const slider = document.getElementById('date-slider');
  slider.max = allDates.length - 2;
